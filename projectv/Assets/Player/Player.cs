...
public class Player : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if(Input.GetKey(KeyCode.LeftArrow) == true)
		{
			...
			rb_RigidBody.MovePosition(transform.position + transform.left * Time.deltaTime * f_Speed);
			...
			{
				rb_RigidBody.MovePosition (transform.position + transform.left * Time.deltaTime * f_RunSpeed);
			}
		}

		//Gets Left Arrow Input to move right
		if(Input.GetKey(KeyCode.RighttArrow) == true)
		{
			//moves the Player to the left using the basic speed
			rb_RigidBody.MovePosition = (transform.position + transform.right * Time.deltaTime * f_Speed);
			
			//Ensure the Speed provided is the Run Speed and not the normal speed
			if(Input.GetKey(KeyCode.LeftShift)) // && Input.GetKey(KeyCode.LeftArrow) == true))
			{
				rb_RigidBody.MovePosition (transform.position + transform.right * Time.deltaTime * f_RunSpeed);
			}
		}
	}
}
